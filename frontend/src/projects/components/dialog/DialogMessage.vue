<template>
  <div>
    <CModal add-content-classes="test" :title="$t('components.dialog_dialogmessage_vue_message_title')" color="danger"
      :show.sync="message.status" :centered="true" :close-on-backdrop="false">
      <template #header-wrapper>
        <div></div>
      </template>
      <template #body-wrapper>
        <CCard class="bg-style2">
          <CCardHeader class="bg-gradient-danger text-white"
            style="border-top-left-radius: 1rem; border-top-right-radius: 1rem ">
            <span class="font-weight-bold h6">
              <CIcon name="cil-speech" size="lg" class="mr-2" /> ERROR
            </span>
          ...
          <CCardBody>
            ...
            <CRow class="text-right mt-3">
              <CCol>
                ...
                <CButton class="ml-2" size="sm" :color="objs.color" shape="pill" variant="outline"
                  @click="onSubmit(objs.code)" v-for="objs in message.button" :key="objs.code">
